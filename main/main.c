...
void app_main(void){
    ...
    */
    xTaskCreate(&run_main, "CallMain", 4096, NULL, 1, &TaskMain);
}
...
void get_mac(){
    ...
    */
    ...
    snprintf(MAC, 20*sizeof(char), "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
...
void mqtt_callback(const char *topic, char *payload) {
    ...
    */
    ...
    if((strcmp(topic,MAC)==0) || (strcmp(topic,"efeito")==0)){
        ...
        xTaskCreate(&callMenu, "CallMENU", 4096, &valorInteiro, 5, &TaskMenu);
    ...
    */
    ...
    switch (event->event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            snprintf(topic, event->topic_len*sizeof(char), "%s", event->topic);
            snprintf(data, event->data_len*sizeof(char), "%s", event->data);

            mqtt_callback(event->topic, event->data);
            break;
